...
public class RandomDog extends PureCommand {
    public static Long time = 0L;

    public RandomDog(String name) {
        super(name);
    }

    @Override
    public String info() {
        return super.info() + " -> 发送修狗图片";
    }
    ...
    public void onCommand(GroupMessageEvent event){
        ...
        if (newTime - time > Config.INSTANCE.getGetRandImageCD()) {
            ...
            String myurl = "url";
            try {
                myurl = ImgDownloader.downloadAndParse("https://random.dog/woof.json",5);
            } catch (IOException e) {
                e.printStackTrace();
            }
